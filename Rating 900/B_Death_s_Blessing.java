...
public class B_Death_s_Blessing {
    static class Reader {
    ...
    public static void main(String args[]) throws IOException {
        ...
        while (t-- > 0) {
            ...
            for (int i = 0; i < n; i++) {
                pair[i] = new Pair(health[i], spell[i]);
            }
            Collections.sort(Arrays.asList(pair), (a, b) -> Long.compare(a.spell, b.spell));
            long time = pair[0].health;
            for (int i = 1; i < n; i++) {
                time += pair[i].health + pair[i - 1].spell;
            }
            println(time);
        }
        out.flush();
    ...
    static class Pair {
        ...
        public Pair(long health, long spell) {
            this.health = health;
            this.spell = spell;
        }
    }
