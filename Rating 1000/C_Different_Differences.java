...
public class C_Different_Differences {
    ...
    public static void main(String args[]) throws IOException {
        ...
        int t = inputInt();
        while (t-- > 0) {
            int k = inputInt(), n = inputInt();
            int maxf = 1;
            for (int f = 1; f < k; f++) {
                if (sumofdiffrernces(f, k - 1) <= n - 1) {
                    maxf = f;
                } else
                    break;
            }
            int differences[] = constrctDifferences(maxf, k - 1);
    ...
    public static int[] constrctDifferences(int f, int k) {
        int arr[] = new int[k];
        for (int i = 0; i < f - 1; i++) {
            arr[i] = i + 2;
        }
    ...
    public static int sumofdiffrernces(int f, int k) {
        int sum = 0;
        for (int i = 0; i < k; i++) {
            if (i < f - 1) {
                sum += i + 2;
